...
class _SupplierPreferencesScreenState extends State<SupplierPreferencesScreen> {
  ...
  void didUpdateWidget(SupplierPreferencesScreen oldWidget) {
    ...
    if (oldWidget.supplier.balance != widget.supplier.balance) {
      _applyVisibilityRule(widget.supplier.balance);
    }
  }
  ...
  Future<void> _loadDetails() async {
    ...
    // تحديث الحالة وتطبيق القاعدة في setState واحدة لضمان التزامن
    setState(() {
      _allTransactions = transactions;
      _isLoading = false;
      _visibleTransactions = (widget.supplier.balance == 0.0)
          ? <Map<String, String>>[]
          : List<Map<String, String>>.from(transactions);
    });

    if (widget.supplier.balance == 0.0 && transactions.isNotEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content:
              Text('الرصيد أصبح صفراً — تم إخفاء سجلات المسحوبات من العرض'),
